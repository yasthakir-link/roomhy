...
export default function WebsiteFastBidding() {
  ...
  useHtmlPage({
    ...
    bodyClass: "bg-white text-slate-950",
    htmlAttrs: {
      lang: "en",
      class: "scroll-smooth"
    },
  ...
  return (
    <div className="html-page min-h-screen">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
          <a href="/website/index" className="flex items-center gap-3 text-lg font-semibold tracking-tight text-slate-950">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-slate-950 text-white">
              <i data-lucide="zap" className="h-5 w-5"></i>
            </span>
            <span>Roomhy Fast Bidding</span>
          </a>
          <a href="/website/index" className="inline-flex items-center gap-2 text-sm font-medium text-slate-600 transition hover:text-slate-950">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:py-10">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <p className="mb-2 text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Fast bidding</p>
            <h1 className="text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">Quick bidding form</h1>
            <p className="mt-3 text-sm leading-6 text-slate-600 sm:text-base">
              Fill in your details, narrow the match, and send bids to the properties that fit your budget and location.
            </p>
          </div>
          <div className="flex gap-3">
            <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Step</p>
              <p className="text-lg font-semibold text-slate-950">1 of 1</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Matches</p>
              <p className="text-lg font-semibold text-slate-950">{propertiesData.length}</p>
            </div>
          </div>
        </div>

        <div className="panel-card mb-8 p-5 sm:p-8">
          <form id="fastBiddingForm" onSubmit={submitBids}>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div>
                <label className="mb-2 block text-sm font-semibold text-slate-900">Gmail Address *</label>
            ...
            <div className="mt-6">
              <div>
                <label className="mb-2 block text-sm font-semibold text-slate-900">Gender *</label>
                ...
                <p className="mt-1 text-xs text-slate-500">This helps us show matching properties.</p>
              </div>
            </div>
            <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                <label className="mb-2 block text-sm font-semibold text-slate-900">Select City *</label>
                ...
                <p className="mt-1 text-xs text-slate-500">Choose from available cities.</p>
              </div>
              <div>
                <label className="mb-2 block text-sm font-semibold text-slate-900">Select Area *</label>
                ...
                <p className="mt-1 text-xs text-slate-500">Choose from available areas.</p>
              </div>
            </div>

            <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                <label className="mb-2 block text-sm font-semibold text-slate-900">Minimum Price Range (₹) *</label>
                ...
                <p className="mt-1 text-xs text-slate-500">Minimum expected rent.</p>
              </div>
              <div>
                <label className="mb-2 block text-sm font-semibold text-slate-900">Maximum Price Range (₹) *</label>
                ...
                <p className="mt-1 text-xs text-slate-500">Maximum expected rent.</p>
              </div>
            </div>
              ...
              {loadingProperties && (
                <div id="propertiesLoadingSpinner" className="loading show flex items-center justify-center py-8">
                  <div className="animate-spin">
                    <i data-lucide="loader" className="h-6 w-6 text-slate-900"></i>
                  </div>
                  <span className="ml-2 text-slate-600">Loading properties...</span>
                </div>
              )}
              {!loadingProperties && propertiesData.length === 0 && (
                <div id="noPropertiesMsg" className="rounded-2xl border border-dashed border-slate-200 py-12 text-center text-slate-500">
                  <i data-lucide="inbox" className="mx-auto mb-3 h-12 w-12 text-slate-300"></i>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="mt-1 text-sm text-slate-500">
                            <i data-lucide="hash" className="mr-1 inline h-3 w-3"></i> Property #{propertyId}
                          </p>
                          <div className="mt-3 flex flex-wrap gap-2 text-xs">
                            <span className="rounded-full bg-slate-100 px-3 py-1 text-slate-700">
                              <strong className="font-semibold text-slate-950">₹{rent}</strong>/month
                            </span>
                            <span className="rounded-full bg-slate-100 px-3 py-1 text-slate-700">{gender}</span>
            ...
            <div className="mt-8 flex flex-col gap-3 border-t border-slate-200 pt-6 sm:flex-row">
              <button
                ...
                className="flex-1 rounded-xl border border-slate-300 bg-white px-4 py-3 font-semibold text-slate-700 transition hover:border-slate-950 hover:text-slate-950"
              >
                Clear Form
              </button>
              <button type="submit" className="btn-primary flex flex-1 items-center justify-center gap-2 rounded-xl px-4 py-3">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div className="info-panel p-6">
            <div className="flex gap-3">
              <i data-lucide="info" className="mt-0.5 h-6 w-6 flex-shrink-0 text-slate-950"></i>
              <div>
                <h4 className="mb-1 font-semibold text-slate-950">How it works</h4>
                <p className="text-sm leading-6 text-slate-600">Your bid goes to all matching property owners. They can review, accept, or respond with a counter offer.</p>
              </div>
            </div>
          </div>
          <div className="info-panel p-6">
            <div className="flex gap-3">
              <i data-lucide="shield" className="mt-0.5 h-6 w-6 flex-shrink-0 text-slate-950"></i>
              <div>
                <h4 className="mb-1 font-semibold text-slate-950">Secure and safe</h4>
                <p className="text-sm leading-6 text-slate-600">Your phone number stays hidden until a property owner accepts your bid. No direct contact before agreement.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div id="submissionModal" className={`fixed inset-0 z-50 items-center justify-center bg-black/50 p-4 ${showSuccessModal ? "flex" : "hidden"}`}>
        <div className="modal-card w-full max-w-md rounded-3xl p-8">
          <div className="text-center mb-6">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full border border-slate-200 bg-slate-50">
              <i data-lucide="check" className="h-8 w-8 text-slate-950"></i>
            </div>
            <h2 className="mb-2 text-2xl font-semibold tracking-tight text-slate-950">Bid sent successfully</h2>
            <p className="text-sm text-slate-600">Your bid has been sent to all matching property owners.</p>
          </div>
          <div className="mb-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <p className="text-sm text-slate-600">
              <strong className="font-semibold text-slate-950">Bids sent to:</strong> <span id="bidCountDisplay">{successCount}</span> properties
            </p>
            <p className="mt-2 text-sm text-slate-600">Property owners will review your bid and respond within 24 hours.</p>
          </div>
          <button onClick={() => setShowSuccessModal(false)} className="btn-primary w-full rounded-xl py-3">
            Done
          </button>
        </div>
      </div>

      <div id="signupModal" className={`fixed inset-0 z-50 items-center justify-center bg-black/50 p-4 ${showSignupModal ? "flex" : "hidden"}`}>
        <div className="modal-card w-full max-w-md overflow-hidden rounded-3xl">
          <div className="border-b border-slate-200 px-6 py-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-2xl font-semibold tracking-tight text-slate-950">Account verification</h2>
                <p className="mt-1 text-sm text-slate-600">Create an account to continue bidding.</p>
              </div>
              <button onClick={() => setShowSignupModal(false)} className="rounded-full border border-slate-200 p-2 text-slate-600 transition hover:border-slate-950 hover:text-slate-950">
          ...
          <div className="px-6 py-8">
            <div className="mb-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-1 text-base font-semibold text-slate-950" id="signupEmailDisplay">
                {signupEmail}
              </p>
            </div>
            ...
            <div className="mb-6">
              <p className="text-center leading-relaxed text-slate-600">
                <span className="font-semibold text-slate-950">No account found</span> for this email address. Please create an account to verify your identity and continue with bidding.
              </p>
            </div>

            <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-4">
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Why sign up?</p>
              <ul className="space-y-2 text-sm text-slate-600">
                <li className="flex items-center">
                  <span className="mr-2 font-bold text-slate-950">✓</span>
                  Verified account
                </li>
                <li className="flex items-center">
                  <span className="mr-2 font-bold text-slate-950">✓</span>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowSignupModal(false)} className="flex-1 rounded-xl border border-slate-300 px-4 py-3 font-semibold text-slate-700 transition hover:border-slate-950 hover:text-slate-950">
                Cancel
              </button>
              <button
                onClick={() => {
                ...
                className="btn-primary flex-1 rounded-xl px-4 py-3"
              >
                Sign Up Now
              </button>
            </div>
