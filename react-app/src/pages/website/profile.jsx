...
import { getWebsiteUser, getWebsiteUserEmail, getWebsiteUserName } from "../../utils/websiteSession";
import { useHeroSlideshow, useWebsiteCommon, useWebsiteMenu } from "../../utils/websiteUi";

const splitUserName = (user) => {
  const firstName = String(user?.firstName || "").trim();
  const lastName = String(user?.lastName || "").trim();
  const fallbackName = String(user?.fullName || user?.name || "").trim();
  const combined = [firstName, lastName].filter(Boolean).join(" ").trim() || fallbackName;
  const parts = combined ? combined.split(/\s+/) : [];

  return {
    firstName: firstName || parts[0] || "",
    lastName: lastName || parts.slice(1).join(" ") || "",
    displayName: combined || getWebsiteUserName() || "Guest"                                            
  };
};

const buildFormState = (user) => {
  const { firstName, lastName, displayName } = splitUserName(user);                                                                                                                                                                                                                                                                                                                                                                  

  return {
    firstName,
    lastName,
    gender: String(user?.gender || "").trim(),
    email: getWebsiteUserEmail() || String(user?.email || "").trim(),
    phone: String(user?.phone || user?.mobile || user?.mobileNumber || "").trim(),
    emergencyName: String(user?.emergencyName || user?.emergencyContactName || "").trim(),
    emergencyPhone: String(user?.emergencyPhone || user?.emergencyContactPhone || "").trim(),
    relationship: String(user?.relationship || "").trim(),
    permanentAddress: String(user?.permanentAddress || user?.address || "").trim(),
    addressProof: String(user?.addressProof || "").trim(),
    profession: String(user?.profession || "").trim(),
    displayName
  };
};

const getProfileInitials = (formData) => {
  const initialsFromName = [formData.firstName, formData.lastName]
    .filter(Boolean)
    .map((part) => part.trim()[0]?.toUpperCase() || "")
    .join("")
    .slice(0, 2);

  if (initialsFromName) return initialsFromName;

  const fallbackName = String(formData.displayName || "").trim();
  if (fallbackName) {
    return fallbackName
      .split(/\s+/)
      .filter(Boolean)
  ...
  const emailPrefix = String(formData.email || "").trim();
  return emailPrefix ? emailPrefix[0].toUpperCase() : "U";
};
...
export default function WebsiteProfile() {
  ...
  const [formData, setFormData] = useState(() => buildFormState(getWebsiteUser()));

  useEffect(() => {
    const syncProfile = () => {
      setFormData(buildFormState(getWebsiteUser()));
    };

    syncProfile();
    window.addEventListener("storage", syncProfile);
    window.addEventListener("roomhy:website-session-changed", syncProfile);

    return () => {
      window.removeEventListener("storage", syncProfile);
      window.removeEventListener("roomhy:website-session-changed", syncProfile);
    };
  }, []);
      ...
      <section className="relative py-20 md:py-28 text-white">
        ...
        <div className="container mx-auto px-4 sm:px-6 text-center relative z-10">
          ...
          <div className="relative w-full max-w-2xl mx-auto">
            <input
              type="text"
              placeholder="Search for 'PG near me' or 'Hostel in Kota'"
            ...
            <button
              type="submit"
              className="absolute right-2.5 top-1/2 -translate-y-1/2 p-2.5 bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors"
            >
          ...
          <div className="flex justify-between items-center px-6 py-2">
            <div className="flex items-center space-x-3">
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-400 to-purple-600 flex items-center justify-center">
                <span className="text-lg font-bold text-white">{getProfileInitials(formData)}</span>
              </div>
              <div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-section p-6 md:p-8">
              ...
              <p className="text-gray-600 -mt-4 mb-6">
                Update your profile information, including name, profile picture, and personal details.
              </p>
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <button
                      type="button"
                      className="flex items-center gap-2 bg-white text-gray-700 px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors text-sm font-medium"
                    >
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select id="gender" name="gender" className="form-input" value={formData.gender} onChange={handleFieldChange}>
            ...
            <div className="form-section p-6 md:p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <select id="relationship" name="relationship" className="form-input" value={formData.relationship} onChange={handleFieldChange}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    name="addressProof"
                    className="form-input"
                    placeholder="Enter proof document type"
            ...
            <div className="p-6 md:p-8 bg-gray-50/50 rounded-b-2xl flex justify-end items-center gap-4">
              <button
                type="button"
                onClick={handleReset}
                className="bg-white text-gray-700 px-5 py-2.5 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors font-medium text-sm"
              >
                Cancel
              </button>
